...
STRATEGY="${STRATEGY:-similarity_topk}"
SHOT_NUM="${SHOT_NUM:-20}"
RUN_ID="${RUN_ID:-qwen-gold-${STRATEGY}-shot${SHOT_NUM}-oof-8b}"
...
KEEP_EXISTING_FOLDS="${KEEP_EXISTING_FOLDS:-1}"

RUN_DIR="exp/few_shot/runs/${RUN_ID}"
LOG_DIR="${RUN_DIR}/logs"
LOG_FILE="${LOG_DIR}/main_experiment_oof.log"
...
{
  echo "[MAIN OOF] Started at $(date '+%Y-%m-%d %H:%M:%S')"
  echo "[MAIN OOF] Strategy: ${STRATEGY}"
  echo "[MAIN OOF] Shot num: ${SHOT_NUM}"
  echo "[MAIN OOF] Run dir: ${RUN_DIR}"
  echo "[MAIN OOF] Model: ${QWEN_MODEL}"
  echo "[MAIN OOF] Embedding model: ${EMBEDDING_MODEL}"
  echo "[MAIN OOF] Folds: ${FOLDS}"
...
for mod_name in mods:
    try:
        ...
        print(f"[MAIN OOF] {mod_name} {getattr(mod, '__version__', '')}")
    except Exception as exc:
        print(f"[MAIN OOF] {mod_name} import failed: {exc}")
PY
  ...
  for fold in ${FOLDS}; do
    ...
    if [[ "${KEEP_EXISTING_FOLDS}" == "1" && -f "${AVG_CSV}" && -f "${AGG_SCORE_JSON}" ]]; then
      echo "[MAIN OOF] Fold ${fold} already has aggregated outputs, skipping"
      continue
    fi

    mkdir -p "${SPLIT_DIR}"

    echo "[MAIN OOF] Preparing fold ${fold}"
    ...
    if [[ "${STRATEGY}" == "random_bootstrap" ]]; then
      ...
      echo "[MAIN OOF] Building random bootstrap input fold ${fold}"
      ...
      echo "[MAIN OOF] Building ${STRATEGY} input fold ${fold}"
    ...
    echo "[MAIN OOF] Inference fold ${fold}"
    ...
    echo "[MAIN OOF] Aggregating fold ${fold}"
  ...
  echo "[MAIN OOF] Merging fold predictions"
...
print("[MAIN OOF] Fold-mean aggregate score")
for key in [
    "ALL_en_ALL_mean",
    "disagree_flag",
    "completeness",
...
print("[MAIN OOF] Full-dev eval_en score")
for key in [
    "ALL-en-ALL-mean",
...
PY
  ...
  echo "[MAIN OOF] Finished at $(date '+%Y-%m-%d %H:%M:%S')"
} 2>&1 | tee "${LOG_FILE}"
