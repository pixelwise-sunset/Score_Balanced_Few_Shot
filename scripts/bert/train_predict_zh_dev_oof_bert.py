#!/usr/bin/env python
"""Train Chinese metric-specific BERT models with dev OOF predictions."""
...
def augment_train_df(df: pd.DataFrame) -> pd.DataFrame:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--official_valid_csv", default="datasets/mediqa-eval-2026-valid_1rater_zh.csv")
...
def main() -> None:
    ...
    for metric in METRICS:
        ...
        for fold in folds:
            ...
            train_df = augment_train_df(metric_df[metric_df["fold"].astype(int) != fold].copy())
            ...
            fold_scores[metric][str(fold)] = metrics_out
            ...
            template.loc[metric_mask, "value"] = [
                value_by_key.get(tuple(row[col] for col in PREDICT_KEY_COLS), row["value"])
                for _, row in template.loc[metric_mask].iterrows()
            ]
            template.loc[metric_mask & (template["value"] >= 0), "rater_id"] = args.strategy_type
    ...
    if (template["value"] < 0).any():
        missing = int((template["value"] < 0).sum())
        raise RuntimeError(f"{missing} dev template rows were not predicted")
    ...
    print(f"Saved prediction CSV to {prediction_csv}", flush=True)
